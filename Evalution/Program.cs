...
app.Run(async (context) =>
{
    ...
    {
        response.Redirect(request.Host.Value);
    }
    else if (request.Path == "/upload" && request.Method == "POST")
    {
        IFormFileCollection files = request.Form.Files;
        // путь к папке, где будут хранитьс€ файлы
        var uploadPath = $"{Directory.GetCurrentDirectory()}/uploads";
        // создаем папку дл€ хранени€ файлов
        ...
        foreach (var file in files)
        {
            ...
            // сохран€ем файл в папку uploads
            using (var fileStream = new FileStream(fullPath, FileMode.Create))
            {
                await file.CopyToAsync(fileStream);
            }
        }
        await response.WriteAsync("‘айлы успешно загружены");
    }
